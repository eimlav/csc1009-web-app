...
import views.Search;
import views.EnterPage;
import views.Independent;
import views.FormSubmitted;
import views.SignUpSuccess;
import views.Play;
import views.LoginUnsuccessfull;
import views.Upload;
import views.LoginSuccessfull;
import views.SignUp;
...
public class Main 
{
	...
	{		
		...
		EnterPage iEnterPage = new EnterPage(databaseInterface,fileStoreInterface);

		Independent iIndependent = new Independent(databaseInterface,fileStoreInterface);

		FormSubmitted iFormSubmitted = new FormSubmitted(databaseInterface,fileStoreInterface);

		SignUpSuccess iSignUpSuccess = new SignUpSuccess(databaseInterface,fileStoreInterface);

		Play iPlay = new Play(databaseInterface,fileStoreInterface);

		LoginUnsuccessfull iLoginUnsuccessfull = new LoginUnsuccessfull(databaseInterface,fileStoreInterface);

		Upload iUpload = new Upload(databaseInterface,fileStoreInterface);

		LoginSuccessfull iLoginSuccessfull = new LoginSuccessfull(databaseInterface,fileStoreInterface);

		SignUp iSignUp = new SignUp(databaseInterface,fileStoreInterface);

		Playlists iPlaylists = new Playlists(databaseInterface,fileStoreInterface);
			...
			for (int i = 0; i < requestToProcess.size(); i++) 
			{
		        ...
		        //the example dynamic page will process the request if the index.html file is not present
		        	if(toProcess.path.length()==0)
		        		toProcess.path = "index.html";		if(iSearch.process(toProcess))
		{
		}
		else
		if(iEnterPage.process(toProcess))
		{
		}
		else
		if(iIndependent.process(toProcess))
		{
		}
		else
		if(iFormSubmitted.process(toProcess))
		{
		}
		else
		if(iSignUpSuccess.process(toProcess))
		{
		}
		else
		if(iPlay.process(toProcess))
		{
		}
		else
		if(iLoginUnsuccessfull.process(toProcess))
		{
		}
		else
		if(iUpload.process(toProcess))
		{
		}
		else
		if(iLoginSuccessfull.process(toProcess))
		{
		}
		else
		...
		if(iPlaylists.process(toProcess))
		{
		}
		else
